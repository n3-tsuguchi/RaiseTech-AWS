...
resource "aws_vpc" "raise_tech_vpc" {
...
resource "aws_subnet" "public_subnet_1a" {
  vpc_id                  = aws_vpc.raise_tech_vpc.id
...
resource "aws_db_instance" "raise_tech_rds" {
  ...
  username              = "admin" 
  password              = jsondecode(aws_secretsmanager_secret_version.rds_master_user_secret_version.secret_string).password # master_password を password に修正
